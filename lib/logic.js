...
async function validateClaim(uClaim) {
    ...
    let orginalFrmHospDt = uClaim.claim.fromHospDt;
    let orginalToHospDt = uClaim.claim.toHospDt;
    ...
    for (let claim of claims) {
        ...
        if (claimantAadharother == originalClaimantAadhar && originalClaimId != claimId && claim.clmStatus == 'APPROVED'
            && claim.fromHospDt>= orginalFrmHospDt && claim.toHospD <= orginalToHospDt){ // need to compare the hosp dates now
    ...
    //claim.claimant = claimant; // loop claimant aadhar undefined, policy id undefined, but loop objects, originalclaim id are coming
    
    originalClaim.lastUpdAgentId = claimId; // checking if claimId is coming
